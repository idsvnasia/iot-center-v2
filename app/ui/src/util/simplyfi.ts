...
const simplifyDist = (xs: number[], ys: number[], epsilon: number) => {
  ...
  const simplifiedXs = (new Float64Array(keptLength) as any) as number[]
  const simplifiedYs = (new Float64Array(keptLength) as any) as number[]
  ...
  return [simplifiedXs, simplifiedYs] as const
}
...
const simplifyDouglasPeuckerHelper = (
  ...
  const indexStack: {i0: number; i1: number}[] = []
...
export const simplifyForNormalizedData = (
  xs: number[],
  ys: number[],
  epsilon: number
) => {
  ...
  return ([__xs, __ys] as any) as [number[], number[]]
}
